...
async function run() {
    try {
        ...
        let deletedJobs = 0; // Variable to keep track of deleted jobs count
        ...
        for (const workflow of workflows) {
            ...
            for (const run of runs) {
                ...
                if (run.status !== "completed") {
                    console.log(`⏭️ Skipped: ${workflow.name} - https://github.com/${repository}/actions/runs/${run.id} - Reason: ${run.status.replace(/_/g, ' ').charAt(0).toUpperCase() + run.status.replace(/_/g, ' ').slice(1)}`);
                    continue;
                }
                if (
                ...
                if (ELAPSE_days >= retain_days) {
                    ...
                    console.log(`⏭️ Skipped: ${workflow.name} - https://github.com/${repository}/actions/runs/${run.id} - Executed: ${run.created_at}`);
                }
            }
            ...
            if (arr_length > 0) {
                ...
                if (keep_minimum_runs !== 0) {
                    ...
                    for (const Skipped of Skip_runs) {
                        console.log(`⏭️ Skipped: ${workflow.name} - https://github.com/${repository}/actions/runs/${Skipped.id} - Executed: ${Skipped.created_at}`);
                    }
                }
